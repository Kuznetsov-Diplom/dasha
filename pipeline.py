# pipeline.py
# Улучшенный пайплайн: 39-мерные признаки (MFCC + Delta + Delta-Delta) + простая нормализация
...
class AudioPipeline:
    """Улучшенный пайплайн (39 признаков)"""
    SAMPLE_RATE = 16000
    ...
    def _extract_mfcc_full(self, y: np.ndarray) -> np.ndarray:
        ...
        return np.vstack([mfcc, delta, delta2])  # (39, n_frames)
    ...
    def extract_raw_39(self, audio_path: str | Path) -> np.ndarray:
        ...
        # Простая нормализация (MinMax по каждому признаку)
