...
def run_feature_experiment(num_runs: int = 8, n_own_phrases: int = 12):
    print(f"🚀 Расширенный эксперимент: {num_runs} итераций, {n_own_phrases} фраз\n")
    ...
    for run in range(1, num_runs + 1):
        ...
        print(f"\n{'='*90}")
        print(f"ИТЕРАЦИЯ {run}/{num_runs} | Свой спикер: {own_speaker[:12]}...")
        print(f"{'='*90}")
        ...
        if len(own_vectors) < 2:
            print("  ⚠️ Слишком мало векторов для своего спикера — пропуск итерации")
            continue

        # Чужие спикеры
        other_speakers = [s for s in all_speakers if s != own_speaker]
        num_foreign = random.randint(14, 20)
        ...
        # Варианты
        variants = [
            ("Полные 26 (mean+std)", own_vectors, foreign_vectors),
            ("Только Mean (13)", own_vectors[:, :13], foreign_vectors[:, :13]),
            ("Только Std (13)", own_vectors[:, 13:], foreign_vectors[:, 13:]),
            ("Первые 8 MFCC (16)", own_vectors[:, :16], foreign_vectors[:, :16]),
            ("Первые 10 MFCC (20)", own_vectors[:, :20], foreign_vectors[:, :20]),
            ("Первые 12 MFCC (24)", own_vectors[:, :24], foreign_vectors[:, :24]),
        ]
    ...
    print("\n" + "="*100)
    print("📊 СВОДНАЯ ТАБЛИЦА (средние по всем итерациям)")
    print("="*100)
...
if __name__ == "__main__":
    run_feature_experiment(num_runs=8, n_own_phrases=12)